...
# get vectors for all unique words and non-unqie words to both sets
spamTop <- as.vector(spamMaster[,1])
legitTop <- as.vector(legitMaster[,1])
spamnotlegit <- c() # words in spam but not legit
spamandlegit <- c() # words in both spam and legit lists
legitnotspam <- c() # words in legit but not spam
...
# get list of words based on the disparity in the probabilty that a word is present in a spam email as compared to a non-spam email
wordVect <- c()
for (i in 1:length(spamandlegit)) {
  ...
  if (abs(diff) > 0.6) { # probability difference of 0.6
